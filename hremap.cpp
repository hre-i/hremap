...
const int HreMapConverter::m_metaBits[] = {
    BIT_LEFTCTRL, BIT_RIGHTCTRL, BIT_LEFTSHIFT, BIT_RIGHTSHIFT,
    BIT_LEFTALT,  BIT_RIGHTALT,  BIT_LEFTMETA,  BIT_RIGHTMETA,
};
...
#define WIN_MOD (BIT_LEFTMETA | BIT_LEFTCTRL | BIT_LEFTSHIFT)
    ...
    if (input->code == KEY_HENKAN) {
      if (input->value == 0) {
        if (m_henkan_only) {
          ...
          // TODO:
          // 同時押しされている(本来の)キーをプレス状態にする必要があるか？
        }
  ...
  case KEY_LEFTBRACE:
    ...
    case 2: // AUTOREPEAT
    case 1: // PRESS
      if (g_enable_ctrl_map && IS_CTRL_ON() && !IS_ALT_ON() && !IS_SHIFT_ON()) {
        DP(("C-[ -> TYPE ESC\n"));
        typeKey(KEY_ESC, 0);
        return true;
      }
